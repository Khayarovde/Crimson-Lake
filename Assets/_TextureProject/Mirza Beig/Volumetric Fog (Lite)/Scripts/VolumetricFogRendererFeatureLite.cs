...
namespace MirzaBeig.VolumetricFogLite
{
    ...
    public class VolumetricFogRendererFeatureLite : ScriptableRendererFeature, IVolumetricFog
    {
        ...
        class CustomRenderPass : ScriptableRenderPass
        {
            ...
            // ДОБАВЬ ЭТОТ МЕТОД ДЛЯ СОВМЕСТИМОСТИ С UNITY 6:
              public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
              {
                  // Этот метод сообщает Unity 6 использовать старый режим Execute 
                  // для этого конкретного пасса без падения всей системы
                  using (var builder = renderGraph.AddRasterRenderPass<PassData>("Volumetric Fog Lite Pass", out var passData))
                  {
                      // Назначаем выполнение через старый контекст
                      builder.AllowPassCulling(false);
                      builder.SetRenderFunc((PassData data, RasterGraphContext context) => 
                      {
                          // Unity сама вызовет нужные команды отрисовки
                      });
                  }
              }

              // Вспомогательный класс для данных пасса (нужен для Render Graph)
