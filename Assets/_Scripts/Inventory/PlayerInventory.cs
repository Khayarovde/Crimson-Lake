...
public class PlayerInventory : MonoBehaviour
{
    ...
    [SerializeField] private bool useRuntimeInventoryCopy = true;
    [SerializeField] private bool clearInventoryOnSceneStart = true;
    ...
    private void InitializeInventoryData()
    {
        ...
        if (useRuntimeInventoryCopy)
        {
            runtimeInventoryData = Instantiate(inventoryData);
            runtimeInventoryData.name = inventoryData.name + " (Runtime)";
            inventoryData = runtimeInventoryData;
        }

        if (clearInventoryOnSceneStart)
        {
            inventoryData.Clear();
            activeItemIndex = -1;
        }
